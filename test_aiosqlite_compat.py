...
async def test_create_function_with_table_data(test_db):
    ...
    async with connect(test_db) as db:
        ...
        # Use in WHERE clause
        results = await db.fetch_all("SELECT value FROM numbers WHERE square(value) > 10")
...
async def test_create_function_multiple_functions(test_db):
    ...
    async with connect(test_db) as db:
        ...
        result = await db.fetch_one(
            "SELECT double(5), triple(5), square(5)"
        )
        assert result[0] == 10
        assert result[1] == 15
        assert result[2] == 25
...
async def test_set_trace_callback_comprehensive(test_db):
    ...
    async with connect(test_db) as db:
        ...
        assert "CREATE TABLE" in sql_text or any("CREATE" in s for s in traced_statements)
...
async def test_custom_function_null_handling(test_db):
    ...
    async with connect(test_db) as db:
        ...
        results = await db.fetch_all("SELECT handle_null(value) FROM test ORDER BY rowid")
        assert results[0][0] == "1"
...
async def test_named_parameters_colon(test_db):
    ...
    async with connect(test_db) as db:
        ...
        rows = await db.fetch_all(
            "SELECT name, age FROM test_params WHERE name = :name",
            {"name": "Alice"}
        )
        assert len(rows) == 1
...
async def test_named_parameters_at(test_db):
    ...
    async with connect(test_db) as db:
        ...
        rows = await db.fetch_all(
            "SELECT name, value FROM test_params WHERE name = @name",
            {"name": "Bob"}
        )
        assert len(rows) == 1
        assert rows[0][0] == "Bob"
...
async def test_named_parameters_dollar(test_db):
    ...
    async with connect(test_db) as db:
        ...
        rows = await db.fetch_all(
            "SELECT key, val FROM test_params WHERE key = $key",
            {"key": "test"}
        )
        assert len(rows) == 1
...
async def test_positional_parameters_question(test_db):
    ...
    async with connect(test_db) as db:
        ...
        await db.execute(
            "INSERT INTO test_params (a, b, c) VALUES (?, ?, ?)",
            ["test", 42, 3.14]
        )
        
        rows = await db.fetch_all(
            "SELECT a, b, c FROM test_params WHERE a = ?",
            ["test"]
        )
        assert len(rows) == 1
        assert rows[0][0] == "test"
...
async def test_positional_parameters_numbered(test_db):
    ...
    async with connect(test_db) as db:
        ...
        await db.execute(
            "INSERT INTO test_params (first, second) VALUES (?2, ?1)",
            [200, "reversed"]
        )
        
        rows = await db.fetch_all(
            "SELECT first, second FROM test_params WHERE first = ?1",
            ["reversed"]
        )
        assert len(rows) == 1
...
async def test_execute_many_with_parameters(test_db):
    ...
    async with connect(test_db) as db:
        ...
        await db.execute_many(
            "INSERT INTO test_execute_many (name, score) VALUES (?, ?)",
            params
        )
        
        rows = await db.fetch_all("SELECT name, score FROM test_execute_many ORDER BY score DESC")
        assert len(rows) == 3
        assert rows[0][0] == "Alice"
        assert rows[0][1] == 95
...
async def test_parameter_sql_injection_prevention(test_db):
    ...
    async with connect(test_db) as db:
        ...
        await db.execute(
            "INSERT INTO test_security (name) VALUES (?)",
            ["normal_name"]
        )
...
async def test_cursor_parameterized_queries(test_db):
    ...
    async with connect(test_db) as db:
        ...
        async with db.cursor() as cursor:
            ...
            await cursor.execute(
                "SELECT name, age FROM test_cursor_params WHERE age > ?",
                [25]
            )
            rows = await cursor.fetchall()
            assert len(rows) == 1
            ...
            await cursor.execute(
                "SELECT name FROM test_cursor_params WHERE name = ?",
                ["Alice"]
            )
...
async def test_transaction_context_manager_with_queries(test_db):
    ...
    async with connect(test_db) as db:
        await db.execute("CREATE TABLE test (id INTEGER PRIMARY KEY, name TEXT, age INTEGER)")
...
async def test_transaction_context_manager_database_error(test_db):
    ...
    async with connect(test_db) as db:
        await db.execute("CREATE TABLE test (id INTEGER PRIMARY KEY, value TEXT UNIQUE)")
        ...
        try:
            async with db.transaction():
                await db.execute("INSERT INTO test (value) VALUES ('test')")  # Duplicate
                await db.execute("INSERT INTO test (value) VALUES ('test2')")  # Should not execute
        except Exception:
...
async def test_transaction_context_manager_multiple_tables(test_db):
    ...
    async with connect(test_db) as db:
        ...
        await db.execute("CREATE TABLE posts (id INTEGER PRIMARY KEY, user_id INTEGER, content TEXT)")
...
async def test_transaction_with_all_query_methods(test_db):
    ...
    async with connect(test_db) as db:
        await db.execute("CREATE TABLE test (id INTEGER PRIMARY KEY, value TEXT, num INTEGER)")
...
async def test_transaction_connection_consistency(test_db):
    ...
    async with connect(test_db) as db:
        ...
        async with db.transaction():
            ...
            await db.execute_many("INSERT INTO test (value) VALUES (?)", [("b",), ("c",)])
...
async def test_row_factory_comprehensive(test_db):
    ...
    async with connect(test_db) as db:
        await db.execute("CREATE TABLE test (id INTEGER PRIMARY KEY, name TEXT, value REAL)")
