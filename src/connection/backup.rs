...
pub(crate) async fn run_backup_loop(
    source_handle: SendPtr<sqlite3>,
    target_handle: SendPtr<sqlite3>,
    name: &str,
    pages: i32,
    sleep: f64,
    progress_callback: Option<Py<PyAny>>,
    backup_busy_timeout_secs: u64,
    source_libversion: String,
) -> Result<(), PyErr> {
    ...
    let name_cstr = std::ffi::CString::new(name.to_string())
        ...
        match step_result {
            SQLITE_OK | SQLITE_BUSY | SQLITE_LOCKED => {
                if (step_result == SQLITE_BUSY || step_result == SQLITE_LOCKED)
                    && backup_start.elapsed() > Duration::from_secs(backup_busy_timeout_secs)
                {
                    unsafe {
