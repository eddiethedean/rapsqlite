...
pytestmark = [pytest.mark.asyncio, pytest.mark.xdist_group("init_hook")]
...
async def test_init_hook_with_pool_size_multiple(tmp_path):
    ...
    async with _isolated_connection(db_path, init_hook=init_hook) as conn:
        ...
        # Verify concurrent operations work (sequential to avoid SQLite lock; pool size 1)
        for i in range(2, 7):
            await conn.execute("INSERT INTO test (id) VALUES (?)", [i])
...
async def test_init_hook_none(tmp_path):
    ...
    async with _isolated_connection(db_path, init_hook=None) as conn:
...
async def test_init_hook_with_pragmas_constructor(tmp_path):
    ...
    async with _isolated_connection(
        db_path, pragmas={"foreign_keys": True}, init_hook=init_hook
    ) as conn:
        # First operation triggers init_hook
        await conn.execute("INSERT INTO test (id) VALUES (1)")
...
async def test_init_hook_multiple_connections(tmp_path):
    ...
    async with _isolated_connection(db_path1, init_hook=init_hook1) as conn1:
        async with _isolated_connection(db_path2, init_hook=init_hook2) as conn2:
...
async def test_init_hook_with_fetch_methods(tmp_path):
    ...
    async with _isolated_connection(db_path3, init_hook=init_hook3) as conn3:
...
async def test_init_hook_with_begin(tmp_path):
    ...
    is visible for the rest of the transaction.
    """
    db_path = tmp_path / "test.db"
    db_path.touch()
...
async def test_init_hook_with_backup(tmp_path):
    ...
    async with _isolated_connection(db_path1, init_hook=init_hook) as conn1:
        async with _isolated_connection(db_path2) as conn2:
            # Trigger init_hook
