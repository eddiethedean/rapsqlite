...
async def test_backup_concurrent_sources(test_db_file, target_db_file):
    ...
    with tempfile.NamedTemporaryFile(suffix=".db", delete=False) as f:
        target_db2 = f.name
    try:
        async with connect(test_db_file) as src1, connect(test_db_file) as src2:
            async with connect(target_db_file) as tgt1, connect(target_db2) as tgt2:
        ...
        async with connect(target_db_file) as verify1, connect(target_db2) as verify2:
...
async def test_backup_source_in_transaction(test_db_file, target_db_file):
    ...
    async with connect(target_db_file) as verify:
...
async def test_backup_sqlite3_target_with_active_transaction(
    ...
    sqlite3_conn = sqlite3.connect(target_db_file)
...
@pytest.mark.asyncio
async def test_backup_sqlite3_target_memory_source_raises():
    """Test backup from memory database to sqlite3.Connection raises error."""
    import sqlite3

    # Create in-memory source and sqlite3 in-memory target (both memory -> raises)
