...
async def test_unicode_named_parameter_uses_utf8_byte_offsets(test_db: str):
    async with connect(test_db) as db:
        assert await db.fetch_all("SELECT :é", {"é": 17}) == [[17]]


async def test_named_parameter_parser_ignores_quoted_sql_and_comments(test_db: str):
    async with connect(test_db) as db:
...
async def test_include_query_in_errors_false_hides_failed_sql(test_db: str):
    async with connect(test_db) as db:
...
async def test_sanitizing_multiple_sensitive_literals_does_not_panic(test_db: str):
    async with connect(test_db) as db:
        with pytest.raises(Exception) as exc_info:
            await db.fetch_all(
...
async def test_leading_sql_comments_do_not_hide_select_results(test_db: str):
    async with connect(test_db) as db:
