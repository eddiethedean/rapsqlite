...
@given(
    value=st.one_of(
        st.integers(min_value=-(2**63), max_value=2**63 - 1),  # Limit to SQLite INTEGER range
...
async def test_parameter_round_trip(test_db, value):
    ...
    async with connect(test_db) as db:
        await db.execute("CREATE TABLE IF NOT EXISTS t (id INTEGER PRIMARY KEY, value BLOB)")
...
async def test_multiple_parameters_round_trip(test_db, values):
    ...
    async with connect(test_db) as db:
        ...
        await db.execute(f"INSERT INTO t ({', '.join([f'c{i}' for i in range(len(values))])}) VALUES ({placeholders})", values)
...
@given(
    table_name=st.text(alphabet=st.characters(min_codepoint=97, max_codepoint=122), min_size=1, max_size=20),
...
async def test_transaction_atomicity(test_db, table_name, count):
    ...
    sql_keywords = {'as', 'select', 'from', 'where', 'insert', 'update', 'delete', 'create', 'table', 'drop'}
    ...
    async with connect(test_db) as db:
        await db.execute(f"CREATE TABLE IF NOT EXISTS {table_name} (id INTEGER PRIMARY KEY, value INTEGER)")
...
async def test_pool_size_invariant(test_db, pool_size, num_operations):
    ...
    async with connect(test_db) as db:
        ...
        await db.execute("CREATE TABLE IF NOT EXISTS t (id INTEGER PRIMARY KEY, value INTEGER)")
...
@settings(max_examples=30, deadline=5000, suppress_health_check=[HealthCheck.function_scoped_fixture])
@given(
    text_value=st.text(max_size=1000)
)
async def test_text_round_trip(test_db, text_value):
    """Test that text values survive round-trip."""
    async with connect(test_db) as db:
        await db.execute("CREATE TABLE IF NOT EXISTS t (id INTEGER PRIMARY KEY, value TEXT)")
...
@settings(max_examples=20, deadline=5000, suppress_health_check=[HealthCheck.function_scoped_fixture])
@given(
    int_value=st.integers(min_value=-(2**63), max_value=2**63 - 1)
)
...
@settings(max_examples=20, deadline=5000, suppress_health_check=[HealthCheck.function_scoped_fixture])
@given(
    blob_value=st.binary(max_size=10000)
)
