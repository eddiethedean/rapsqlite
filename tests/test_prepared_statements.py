...
async def test_parameterized_query_caching():
    ...
    try:
        async with connect(test_db) as conn:
            ...
            elapsed = end_time - start_time
            assert elapsed < 0.5, f"3 parameterized queries took {elapsed:.3f}s, expected < 0.5s"
    finally:
        cleanup_db(test_db)
...
async def test_execute_many_caching():
    ...
    try:
        async with connect(test_db) as conn:
            ...
            elapsed = end_time - start_time
            assert elapsed < 0.5, f"execute_many with 5 params took {elapsed:.3f}s, expected < 0.5s"
