...
async def test_connection_pool_heavy_load(test_db):
    ...
    async def pool_worker(worker_id: int):
        async with connect(test_db) as db:
            ...
            rows = await db.fetch_all("SELECT value FROM t WHERE value = ?", [worker_id])
            return len(rows) == 1
...
async def test_concurrent_connections_stress(test_db):
    ...
    async def stress_worker(worker_id: int):
        ...
        async with connect(test_db) as db:
            for i in range(10):
                await db.execute("INSERT INTO t (value) VALUES (?)", [worker_id * 10 + i])
            rows = await db.fetch_all("SELECT COUNT(*) FROM t WHERE value >= ? AND value < ?", 
                                     [worker_id * 10, (worker_id + 1) * 10])
            return rows[0][0]
    
    # Run 20 concurrent workers
    results = await asyncio.gather(*[stress_worker(i) for i in range(20)], return_exceptions=True)
