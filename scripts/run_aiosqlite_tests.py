...
def clone_aiosqlite(temp_dir: Path) -> Path:
    ...
    try:
        subprocess.run(
            ["git", "clone", "--depth", "1", "https://github.com/omnilib/aiosqlite.git", str(aiosqlite_dir)],
            check=True,
            capture_output=True
        )
...
def patch_imports(content: str) -> str:
    ...
    content = re.sub(r'^import aiosqlite\s*$', 'import rapsqlite as aiosqlite', content, flags=re.MULTILINE)
    ...
    while i < len(lines):
        ...
        if re.match(r'^from aiosqlite import', line):
            ...
            patched_lines.append(line.replace('from aiosqlite import', 'from aiosqlite import'))
...
def run_tests(patched_dir: Path, project_root: Path) -> Tuple[List[str], List[str], List[str]]:
    ...
    try:
        import rapsqlite
        print_status(f"✅ rapsqlite already installed (version: {getattr(rapsqlite, '__version__', 'unknown')})", GREEN)
    except ImportError:
        # Install rapsqlite
        ...
        try:
            ...
            try:
                ...
                print_status(f"⚠️  Could not install rapsqlite", YELLOW)
...
def generate_report(passed: List[str], failed: List[str], skipped: List[str], 
                    project_root: Path, rapsqlite_version: str):
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="rapsqlite_aiosqlite_") as temp_dir:
        ...
        # Generate report
        report_file = generate_report(passed, failed, skipped, project_root, rapsqlite_version)
