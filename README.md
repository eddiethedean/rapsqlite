...
- ✅ **0.5** – Released as `v0.5.0`, with the `v0.5.1` maintenance release published to PyPI: measured low-latency execution, hot-path reductions, scalar/BLOB and prepared-query paths, opt-in session affinity, an opt-in raw path, and connection-lifecycle hardening
